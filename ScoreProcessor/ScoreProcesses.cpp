		...
		while(true)
		{
			...
			public:
				Scaler(unsigned int output_x,unsigned int scale_factor,unsigned int input_dim,unsigned int output_dim):
					...
					input(new float[input_dim]),
					output(new float[output_dim])
				{}
			private:
				...
				void write_to_img(Img* const out,size_t const y) const
				{
					...
					for(size_t r=0;r<output_dim;++r,in_row+=output_dim,out_row+=out_width)
					{
			...
			{
				return upscaled.get_resize(desired_width,desired_height);
			}
